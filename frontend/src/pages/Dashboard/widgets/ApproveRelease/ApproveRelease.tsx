...
import { useApproveRelease } from './hooks';

// prettier-ignore
const styles = {
    container: 'approve-release flex flex-col gap-4',
    actions: 'approve-release-actions flex justify-between items-center gap-3 flex-wrap',
    select: 'approve-select bg-primary/60 border border-secondary rounded-md px-3 py-2 text-sm text-primary focus:outline-none focus:border-primary transition-all duration-200',
...
export const ApproveRelease = () => {
    const { network } = useGetNetworkConfig();
    const { slots, isLoading, error, refetch } = useGetSlots();
    const { approveRelease, isSubmitting } = useApproveRelease();
        ...
        () =>
            slots.filter(
                (slot) =>
                    ...
                    !slot.approvals?.includes(address) &&
                    slot.creator !== address
            ),
        [slots, address]
    );
    ...
    const handleApprove = async () => {
        ...
        const sessionId = await approveRelease(selectedVersion);
    ...
    return (
        <div id={ItemsIdentifiersEnum.approveRelease} className={styles.container}>
            <div className={styles.actions}>
                <div className={styles.selectWrapper}>
                    <Label>Version to approve</Label>
                    <select
            ...
            <OutputContainer isLoading={isLoading}>
                ...
                {!error && proposedSlots.length === 0 && !isLoading && (
                    <p className={styles.empty}>No proposed releases to approve.</p>
                )}

                {!error && selectedSlot && (
                    <div className={styles.infoCard} data-testid='approveReleaseDetails'>
